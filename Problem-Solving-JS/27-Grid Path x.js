...
const minReplacements = i => Math.min(
    ...((g => {
        const n = g.length;
        return [
        ...g.map(r => [...r].filter(c => c === '.').length),
        ...Array.from({ length: n }, (_, c) => g.reduce((a, r) => a + (r[c] === '.'), 0)),
        g.reduce((a, r, j) => a + (r[j] === '.'), 0),
        g.reduce((a, r, j) => a + (r[n - 1 - j] === '.'), 0)
        ];
    })(i.trim().split('\n')))
);